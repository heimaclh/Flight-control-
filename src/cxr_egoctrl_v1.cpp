...
nav_msgs::Odometry position_msg;//读取无人机位置的消息
geometry_msgs::PoseStamped target_pos;//读取rviz打的点
mavros_msgs::State current_state;
float position_x_begin, position_y_begin, position_z_begin, yaw_begin;
bool get_first_pos = false;//是否获取初始位置的标志位，第一次获取后置true
float position_x, position_y, position_z,  current_yaw, targetpos_x, targetpos_y;//没有targetpos_z因为rviz打点只需要xy平面位置
...
//读取ego里的位置速度加速度yaw和yaw_dot（偏航角速度）信息，其实只需要ego的位置速度和yaw就可以了
...
int main(int argc, char **argv)
{
	...
	("move_base_simple/goal", 10, target_cb);//订阅rviz打点的话题
	ros::Subscriber position_sub=nh.subscribe<nav_msgs::Odometry>
    ("/iris_0/mavros/local_position/odom",10,position_cb);//读取无人机位置和姿态的话题
